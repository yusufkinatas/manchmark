...
import { View, Text, Easing, Image, StyleSheet } from 'react-native'
...
export default class GameResult extends Component {
  ...
  componentDidMount() {
    if (this.state.highScore == null || this.state.highScore < this.state.currentScore) {
      user
        .updateHighscore(this.state.currentScore, this.game.hsName)
        .then(() => {
          user
            .getRank(this.game.hsName)
            .then((res) => {
      ...
      <View style={Generics.container}>
        <View style={{ ...Generics.container, marginBottom: 50 }}>
          {this.isHighScore() && (
            <DelayedView
              delay={0}
              style={{ ...styles.infoContainer, justifyContent: 'center', paddingTop: 10 }}
            >
          ...
          >
            <View style={{ width: '100%' }}>
              ...
              >
                <Text style={{ ...Generics.bigText, fontWeight: 'bold', fontSize: 25 }}>
                  {translate('score')}
                </Text>
                <Text style={{ ...Generics.bigText, fontWeight: 'bold', fontSize: 25 }}>
          ...
          >
            <Text style={{ ...Generics.bigText, fontWeight: 'bold', flex: 1, textAlign: 'left' }}>
              {translate('highscore')}
            </Text>
            <Text style={{ ...Generics.bigText, fontWeight: 'bold' }}>
              {this.isHighScore() ? this.state.currentScore : this.state.highScore}
            </Text>
          </DelayedView>
          <DelayedView
            delay={400}
          ...
          >
            <Text style={{ ...Generics.bigText, fontWeight: 'bold', flex: 1, textAlign: 'left' }}>
              {translate('averageOfPlayers')}
            </Text>
            <Text style={{ ...Generics.bigText, fontWeight: 'bold' }}>
              {utils.truncateFloatingNumber(user.get().globalAverages[this.game.hsName], 2)}
            </Text>
          </DelayedView>
          ...
          {this.isHighScore() && user.get().isConnected && (
            ...
            >
              <Text style={{ ...Generics.bigText, fontWeight: 'bold' }}>
                {translate('yourScoreIsHigherThanXOfPlayers').replace(
                  '2',
                  utils.truncateFloatingNumber(
                    100 - (this.state.rank / this.state.userCount) * 100,
                    2
                  )
                )}
              </Text>
        ...
        <DelayedView delay={800} style={{ position: 'absolute', bottom: 20 }}>
          <Text style={{ ...Generics.text, color: colors.secondaryLight2 }}>
            {translate('repetitionIsKeyToSuccess')}
          </Text>
        </DelayedView>
      </View>
    )
    ...
    return (
      <View style={Generics.container}>
        {this.state.isLoading ? (
          <LoadingIndicator text={translate('loading')} color={this.game.backgroundColor} />
        ) : (
          this.renderContent()
        )}
      </View>
