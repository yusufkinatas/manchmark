...
export default class FollowsScreen extends Component {
    ...
    if (_text == '') {
      ...
      if (!validator.isAlphanumeric(text)) {
        this.setState({
          renderStatus: 'search',
          searchText: _text,
          errorText: translate('errOnlyEngLetters'),
        })
      } else {
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity
          ...
          onPress={() => this.onUserPress(user, userFromSearchResults)}
  ...
  renderFollows = () => {
    return (
      <ScrollView
        style={{ width: _SCREEN.width, marginBottom: 0 }}
      ...
      >
        <View>
          <Text style={Generics.text}>
            {translate('xUsersFound').replace('2', this.state.searchResults.length)}
          </Text>
        </View>
          ...
          <View>
            ...
            >
              ...
              {this.state.searchText != '' && (
                <TouchableOpacity
                  onPressIn={() => this.setState({ searchText: '', renderStatus: 'follows' })}
                  activeOpacity={1}
                >
