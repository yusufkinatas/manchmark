...
export default class NumberMemoryGame extends Component {
    ...
    user.set(
      {
        statistics: {
          ...
          ['NumberMemoryGame']: {
            ...
            totalNumberMemorized:
              oldUserStat.NumberMemoryGame.totalNumberMemorized + numberMemorized,
          },
        },
      },
      true
  ...
  onPress = (text) => {
    if (text == 'del') {
      this.setState({
        userAnswer: this.state.userAnswer.slice(0, this.state.userAnswer.length - 1),
      })
    } else if (text == 'enter') {
      this.onAnswer()
    } else {
  ...
  renderMemorizingPhase = () => {
    return (
      <View style={{ justifyContent: 'center', alignItems: 'center' }}>
        ...
        <CounterBar
          time={(this.numberLength + 3) * 1000 + 450}
  ...
  renderGame = () => {
    return (
      <View style={Generics.container}>
        {this.state.isGuessing ? this.renderAnswerPhase() : this.renderMemorizingPhase()}
      </View>
    )
  }
    ...
    return (
      <Text
        style={{
          ...
          fontSize: 24,
          paddingHorizontal: 0,
          color: correct ? colors.secondaryLight3 : gameColor,
  ...
  showError = () => {
    ...
    return (
      <View style={{ ...Generics.container, flex: 0.25, justifyContent: 'center', paddingTop: 10 }}>
        <Text style={{ ...Generics.bigText, fontSize: 16, color: colors.secondaryLight2 }}>
          {translate('number')}
        </Text>
        <Text style={{ ...Generics.bigText, fontSize: 24 }}>{num}</Text>
        <Text style={{ ...Generics.bigText, fontSize: 16, color: colors.secondaryLight2 }}>
          {translate('yourAnswer')}
        </Text>
