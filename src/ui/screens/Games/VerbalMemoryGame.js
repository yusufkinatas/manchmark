...
export default class VerbalMemoryGame extends Component {
  ...
  showAWord = () => {
    ...
    if (
      this.state.score < 1 ||
      (this.usedWords.length > 7 ? Math.random() > 0.5 : Math.random() > 0.35)
    ) {
  ...
  renderGame = () => {
    return (
      <View style={Generics.container}>
        <Text style={{ ...Generics.bigText, color: colors.secondaryLight }}>
          {translate('wasThisShownPreviously')}
        </Text>
        <View style={{ paddingBottom: 20 }} />
        <SwappingText style={Generics.hugeText}>
          {this.state.word && this.state.word[0].toUpperCase()}
          {this.state.word.slice(1)}
        </SwappingText>
        <View style={styles.answerButtonsContainer}>
          <TouchableOpacity
            disabled={this.firstWord}
            onPress={() => this.onAnswer('seen')}
            style={{
              ...styles.answerButton,
              opacity: this.firstWord ? 0.2 : 1,
              backgroundColor: gameColor,
            }}
          >
          ...
          <TouchableOpacity
            onPress={() => this.onAnswer('new')}
            style={{ ...styles.answerButton, marginLeft: 0, backgroundColor: gameColor }}
          >
  ...
  renderFinish = () => {
    return (
      <GameResult onRestart={this.reinitialize} game="VerbalMemoryGame" score={this.state.score} />
    )
  }
